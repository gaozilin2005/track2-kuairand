...
## 2026-08-30 — 学习式集成（stacking）：确认无收益，但解释了为什么

**Hypothesis.** 到目前为止试过的所有融合方式（raw/z-score/rank/RRF/贪心加权）都是
**全局固定权重**——不管哪一行，同一个成员永远用同一份权重。Stacked generalization
（Wolpert 1992）文献的核心论点：学出来的元模型可以让权重**依输入而变**（例如
"BST 在历史丰富的用户上更可信，FM 在冷用户上更可信"），固定权重天生做不到这个。

**Code diff.** 新增 `ablation_stacked_ensemble.py`：把 6 个成员（bst / deepfm /
finalmlp / fm_quantile / fm_watchtime / lightgcn，全部从 `scores/*.npz` 直接加载，
不需要重新训练）的分数当 6 维输入，喂给线性组合器和一个 6→16→1 的小 MLP，用
BPR pairwise 目标训练（跟评测指标对齐，不是普通 pointwise 分类）。

**防泄漏设计**：这次不是从几个离散方案里选，而是真的在拟合参数，"只在 valid 上选，
test 只报一次"这条规矩本身不够了——valid 用来拟合参数后就有信息泄漏风险。所以把
valid **按用户**切成 meta_train（70%，87,171 行/15,663 用户）/ meta_dev（30%，
37,738 行/6,714 用户），在 meta_train 上训、meta_dev 上早停，test 全程不参与
任何决策，最后报一次。

**Metrics.**

| 配置 | meta_dev primary | valid（全量） | test |
|---|---|---|---|
| 等权 z-score 平均（不学习，对照） | — | 0.6064 | 0.6020 |
| 线性组合器 | 0.6092 | 0.6073 | 0.6026 |
| MLP（6→16→1） | 0.6084 | 0.6072 | 0.6019 |
| 线性 + L1(0.02) | 0.6092 | 0.6080 | 0.6028 |
| 线性 + L1(0.05) | 0.6091 | 0.6080 | 0.6028 |
| 线性 + L1(0.1) | 0.6091 | 0.6074 | 0.6026 |
| **对照：静态加权集成（子集搜索，此前最优）** | — | **0.6091** | **0.6034** |

**Finding：全线没有超过静态加权集成，且能解释原因。**

1. **学出来的权重几乎是均匀的**（无 L1 时：bst=0.097, deepfm=0.089,
   finalmlp=0.091, fm_quantile=0.089, fm_watchtime=0.088, lightgcn=0.067）——
   完全没有重现静态方法通过穷举搜索找到的稀疏子集 `{bst, fm_quantile,
   fm_watchtime}`（该子集显式排除了 deepfm/finalmlp/lightgcn）。

2. **L1 正则化也没能找回那个子集**。哪怕加到 0.1 这么强的惩罚，除了 lightgcn
   之外的权重几乎是**成比例一起收缩**，deepfm/finalmlp 从没被压到接近零。
   原因：L1 依据的是"这个特征还能不能降低训练 loss"，静态方法排除某个成员
   依据的是"加入它是否会让**验证集**上的排序指标变差"——这是两个不同的判据。
   deepfm/finalmlp 在 meta_train 上仍然能让 pairwise loss 进一步下降（哪怕这点
   下降不能泛化到 test），L1 天然看不到"泛化变差"这件事，只看得到"训练 loss
   还在降"，所以不会把它们压成零。

3. **meta_dev 分数（0.6091-0.6092）几乎精确复现了静态方法的 valid 分数
   （0.6091），但 test 迁移始终差 0.0006-0.0008。** 组合器只用了 valid 的 70%
   （87K 行）来拟合参数；静态方法的穷举搜索是在**全部** 124,909 行 valid 上
   直接评估每个候选方案的排序指标。对这个问题（候选方案本身自由度很低——
   4 种融合方式 × 至多 63 种子集组合，一共 252 个离散选项）来说，"在更大的
   评估集上从少量离散候选里选"比"在更小的切分上拟合连续参数"更稳。

**跟文献的关系**：stacking 理论上的优势（依输入调整权重）是真实存在的，但
它要求训练数据能撑得起"学习"这件事——这里可学的参数本身很少（6-13 个），
瓶颈不是模型表达能力不够，是**用来拟合的数据切分比穷举搜索能用的数据小**，
且穷举搜索的评判标准（样本外排序指标）比 L1 的判据（训练 loss 大小）更直接
对齐真正要优化的东西。这是一个关于"什么时候学习式方法反而不如穷举搜索"的
具体例子，不是对 stacking 本身的否定。

**Decision.** 不采用，保留静态加权集成（`{bst, fm_quantile, fm_watchtime}`，
valid 0.6091 / test 0.6034）作为最优提交配置。`ablation_stacked_ensemble.py`
留在仓库里作为归因记录。
...
## 2026-08-30 — LightGCN 深挖：确认是结构性天花板，不是欠训练

**Hypothesis.** 连续四个空结果（stacking、画像/统计特征单模型、当集成成员）之后，
用户明确选择把剩余时间押在这一条上：LightGCN 是集成里唯一一个跟 FM 系机制级
不相关的成员（相关性 0.56~0.59，两次独立确认），但单独分数太弱（0.5576），
贪心搜索给它的权重永远是 0。假设：这是训练不足，不是天花板——原来的成员训练
用 patience=4 提前停了，也许再练久一点/调个学习率就能追上来，够到能被集成用上
的水平。

**Code diff.** `lightgcn_model.py` 补了 `--patience` CLI 参数（之前硬编码在
`run_lightgcn` 里但没暴露出来，测的时候才发现传不进去）。

**实验 1：关掉早停，跑满 40 epoch。**

| epoch | valid primary | 备注 |
|---|---|---|
| 1 | 0.5552 | |
| 8 | 0.5643 | 原早停成员（patience=4）大致停在这附近 |
| 14 | 0.5659 | |
| 40 | 0.5668 | 32 个额外 epoch 只换来 +0.0025 |

test primary 在 epoch 40 是 **0.5591**，只比原来早停版本（0.5576）高 0.0015——
5 倍的训练量换来几乎可以忽略的提升。**从 epoch 10~14 开始基本走平**，后面
26 个 epoch 只是缓慢地在噪声范围内漂移。

**实验 2：学习率从 0.001 提到 0.01（会不会是卡在一个差的局部解，换个学习率
跳出来）。** 结果更清楚：epoch 4 就到了 valid primary **0.5654**——跟 lr=0.001
练 8 个 epoch 到的地方几乎一样，只是更快到而已；紧接着从 epoch 5 开始**单调
过拟合**（train loss 一路降到 0.24，valid primary 一路跌到 0.547）。两个完全
不同的学习率，练出来的天花板是同一个数字。

**Finding：两个独立操纵变量（训练时长、学习率）都指向同一个结论——
天花板是结构性的，不是没炼到位。** 真正的原因很直接：LightGCN 在这个实现里
**只吃得下 `user_id`/`video_id`**，拿不到 `author_id`/`tab`/`duration_ms`/
两个时序特征——恰恰是把 FM 从 0.5946 推到 0.6017 的那些东西。它的弱不是"图
传播这个机制不行"，是"图传播这个机制目前只被喂了两个域"。

**这也解释了为什么给它加特征这条路走不通**：`side_video` 那条记录已经证明——
决定集成价值的是**计算机制**，不是数据来源。如果给 LightGCN 也接上 7 个域，
它多半会像 `side_video` 一样迅速跟 FM 系拉到 0.85+ 的相关性（因为此时两边
喂的信息几乎一样，只是传播方式不同，而传播方式本身在这个稀疏度下似乎不能
产生足够独立的误差模式来抵消"信息来源趋同"的效应）——这个猜测本身值得验证，
但代价是要重新设计 LightGCN 的输入端（图节点从纯 user/item 扩展到能承载多域
信息的形式，不是加几行代码的事），投入产出比在当前时间预算下不划算。

**Decision.** LightGCN 的定位到此为止：**确认是"机制互补但先天信息不足"**，
不是"调参没调对"。不再对它投入更多训练时间/超参搜索。它作为集成成员的角色
维持"贪心搜索给零权重"的现状，最优提交配置不变（valid 0.6091 / test 0.6034）。

**关于这次研究推进的整体结论**：本轮从"stacking"到"官方画像/统计特征"到
"LightGCN 深挖"一共五个独立方向，全部是空结果，但每一个都排除了一种可能性、
补上一块机制理解（stacking 的"搜索自由度 vs 泛化"、side_features 的"数据来源
新不代表机制新"、LightGCN 的"结构性天花板 vs 欠训练"）。在当前特征集和已经
尝试过的机制范围内，剩余方向的期望值已经很低——继续搜索大概率是同一类结果的
重复。

Reproduce: `python3 lightgcn_model.py --epochs 40 --patience 100`（confirms
plateau）；`python3 lightgcn_model.py --epochs 20 --patience 100 --lr 0.01`
（confirms same ceiling reached faster, then overfits）。
