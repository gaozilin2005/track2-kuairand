...
def main():
    ...
    while it < a.max_iterations:
        ...
        log_text = "\n".join(log_lines)
        try:
            action, usage, cost = call_agent(log_text, a.model, a.max_budget_usd)
    ...
    # final confirmation: re-run the best-found config across multiple seeds
    successes = [h for h in history if not h["failed"]]
    summary = ["\n---\n## Run summary\n",
               f"- Iterations run: {it}\n",
    ...
    if successes:
        ...
        summary.append(f"  single-seed valid primary {best['valid_primary']:.4f}, test primary {best['test_primary']:.4f}\n")

        seed_results = []
        for s in range(a.final_seeds):
            m, dt, err, cmd = run_baseline(best["action"], s, a.data_dir, a.epochs)
            if m:
        ...
        if seed_results:
            ...
            summary.append(f"- Final {len(seed_results)}-seed confirmation: "
                           f"valid {st.mean(vv):.4f} +/- {st.pstdev(vv):.4f}, "
                           f"test {st.mean(tv):.4f} +/- {st.pstdev(tv):.4f}\n")
            summary.append(f"  vs. official baseline test primary 0.5946: "
                           f"{'BEATS' if st.mean(tv) > 0.5946 else 'DOES NOT BEAT'} baseline "
                           f"(delta {st.mean(tv)-0.5946:+.4f})\n")
    else:
